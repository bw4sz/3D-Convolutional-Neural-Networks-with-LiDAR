...
loader = tf.train.import_meta_graph("H:\\Temp\\Chapter_2_bits\\Model_Testing\\Old_Model\\lenet_317343.0_301055.0_45-13800.meta")
loader.restore(sess, "H:\\Temp\\Chapter_2_bits\\Model_Testing\\Old_Model\\lenet_317343.0_301055.0_45-13800")
...
try:
   pred1=graph.get_tensor_by_name("add_2:0")
except:
   pass
...
#summary file
fd = open('H:/Temp/convnet_3d/Lenet/summarized.csv','a')
...
for i in range(1500000):
    ...
    if i%100==0:
        ...
        for n in range(40):
            ...
            #training is set to true because I messed up this model and didn't save batch_norm weights. With a large batch size it should be fine.
            #If you've trained a new model you'll want training to be false
            [valid_acc, pred]= sess.run([mse, pred1] ,feed_dict={xs0: batch_valid_xs, ys0: batch_valid_ys, training: True})
            valid_accuracies.append(valid_acc)
        ...
        if i>1000:
            if valid_acc2 < record_low:
                ...
                #save new model
                saver.save(sess, 'H:\\Temp\\Chapter_2_bits\\Model_Testing\\Old_Model\\Lenet_retrained_'+str(train_acc)+'_'+str(valid_acc2)+'_'+str(RMSE), global_step=i)
                overtrain_indicator=0
            else:
